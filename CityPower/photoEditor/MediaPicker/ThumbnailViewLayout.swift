...
final class ThumbnailsViewLayout: UICollectionViewLayout {
    ...
    override func prepare() {
        ...
        for index in 0..<itemsCount {
            ...
            let frame = CGRect(
                x: sectionInset.left  + CGFloat(index) * (width + spacing),
                y: collectionViewBounds.midY - width / 2,
                width: width,
                height: height
            )
            ...
            cellAttributes.center = CGPoint(x: frame.midX, y: frame.midY)
            
            attributes[indexPath] = cellAttributes
            maxX = frame.maxX
        }
        
        contentSize = CGSize(width: maxX + sectionInset.right, height: collectionViewBounds.height)
    ...
    private func startDragAtLocation(location: CGPoint) {
        ...
        if let draggingView = draggingView {
            ...
            UIView.animate(withDuration: 0.28, animations: {draggingView.transform = self.itemsTransform.scaledBy(x: 1.1, y: 1.1) }, completion: nil)
    ...
    private func indexPathForItemClosestTo(point: CGPoint) -> IndexPath? {
        ...
        for attribute in layoutAttributes {
            ...
            let currentDistance = abs(attribute.frame.minX - point.x)
    ...
    private func moveItem(to location: CGPoint) {
        ...
        if let newIndexPath = indexPathForItemClosestTo(point: CGPoint(x: location.x, y: collectionView.bounds.height/2)),
    ...
    @objc private func onContinuousScroll() {
        ...
        draggingView.frame.origin.x += scrollRate
